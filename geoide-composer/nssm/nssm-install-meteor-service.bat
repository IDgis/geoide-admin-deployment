...
echo NSSM=%NSSM_% SRV=%SERVICE_NAME_% MAIN=%METEOR_MAIN_FOLDER_% PROG=%METEOR_PROGRAM_NAME_%\nssm\nssm-start-meteor-service.bat PORT=%METEOR_PORT_% DB=%MONGO_DB_NAME_%  
%NSSM_% install %SERVICE_NAME_% %METEOR_MAIN_FOLDER_%\%METEOR_PROGRAM_NAME_%\nssm\nssm-start-meteor-service.bat %METEOR_PORT_% %MONGO_DB_NAME_%  
rem %NSSM_% set <servicename> ObjectName <username> <password> WERKT NIET!!!!
%NSSM_% set %SERVICE_NAME_% AppDirectory %METEOR_MAIN_FOLDER_%\%METEOR_PROGRAM_NAME_%\meteor
%NSSM_% set %SERVICE_NAME_% DisplayName %METEOR_PROGRAM_NAME_%
%NSSM_% set %SERVICE_NAME_% Description "Meteor service %METEOR_PROGRAM_NAME_%"
%NSSM_% set %SERVICE_NAME_% Start SERVICE_AUTO_START
%NSSM_% set %SERVICE_NAME_% AppStdout %METEOR_MAIN_FOLDER_%\%METEOR_PROGRAM_NAME_%\logs\out.log
%NSSM_% set %SERVICE_NAME_% AppStderr %METEOR_MAIN_FOLDER_%\%METEOR_PROGRAM_NAME_%\logs\err.log
rem Do not start the service because it is installed under LocalSystem
rem This will have to be remedied by hand to use current user and password
rem %NSSM_% start %SERVICE_NAME_% 
